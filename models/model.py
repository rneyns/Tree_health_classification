...
class FeedForward(nn.Module):
    def __init__(self, dim, mult = 4, dropout = 0.):
        ...
        self.net = nn.Sequential(
            nn.Linear(dim, dim * mult * 2, dtype=torch.float32),
            GEGLU(),
            nn.Dropout(dropout),
            nn.Linear(dim * mult, dim, dtype=torch.float32)
        )
...
class Attention(nn.Module): ## this is where the attention module is actually implemented, so elegant... so simple...
    def __init__(
        ...
        self.to_qkv = nn.Linear(dim, inner_dim * 3, bias = False, dtype=torch.float32) ## A linear layer that projects the input feature vector into queries, keys, and values. The output dimension is three times the inner_dim because it includes separate projections for queries, keys, and values concatenated together.
    ...
    def forward(self, x, mask = None):
        ...
        if mask is not None:
            # Ensure mask has the correct shape (batch_size, 1, 1, sequence_length)
    ...
    def __init__(self, num_tokens, dim, nfeats, depth, heads, dim_head, attn_dropout, ff_dropout,style='col'):
        ...
        for _ in range(depth):
            if self.style == 'colrow':
                print(f"dim: {dim}; nfeats: {nfeats}; heads = {heads}; dim_head = {dim_head}")
...
class MLP(nn.Module):
    def __init__(self, dims, act = None):
        ...
        for ind, (dim_in, dim_out) in enumerate(dims_pairs):
            ...
            linear = nn.Linear(dim_in, dim_out, dtype=torch.float32)
