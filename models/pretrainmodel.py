...
class SAINT(nn.Module):
    def __init__(
            ...
            *,
            ...
            depth,
            ## the depth of the transformer model --> how many transformer blocks (multi-head self-attention + MLP) are stacked on top of eachother --> default = 6
            ...
            scalingfactor=10,
            attentiontype='col',
            # attentiontype is default on col so dubbelcheck if this is still the case in my experiments
        ...
        self.pt_mlp = simple_MLP(
            [dim * (self.num_continuous), 6 * dim * (self.num_continuous) // 5, dim * (self.num_continuous) // 2])
        self.pt_mlp2 = simple_MLP(
            [dim * (self.num_continuous), 6 * dim * (self.num_continuous) // 5, dim * (self.num_continuous) // 2])

    def forward(self, x_categ,
                x_cont):  ##this is actually not used + so many other things in this script are not used --> would love to clean it up
