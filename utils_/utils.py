...
def class_wise_acc_(model, dloader, device):
    ...
    with torch.no_grad():
        for i, data in enumerate(dloader, 0):
            image, ids, DOY, x_categ, x_cont, y_gts = data[0].to(device), data[1].type(torch.float32).to(device), data[2].type(torch.float32).to(
                device), data[3].type(torch.float32).to(device), data[4].type(torch.float32).to(
                device),data[5].to(device, dtype=torch.long)#,data[6].to(device).type(torch.float32)
